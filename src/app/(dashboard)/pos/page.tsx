...
export default function POSPage() {
  ...
  const addItem = (item: { id: string; name: string; selling_price: number }) => {
    setCart((prev) => {
  ...
  const handleProcessCashPayment = () => {
    ...
    clearCart();
    setGlobalDiscount(0);
    toast.success("Transaksi tunai sukses diproses!");
  };
  ...
  const handleProcessQrisPayment = () => {
    ...
    clearCart();
    setGlobalDiscount(0);
    toast.success("Pembayaran QRIS berhasil divalidasi!");
  };
  ...
  return (
    <div className="flex h-[calc(100vh-4rem)] w-full flex-col bg-[#fffdfa] md:flex-row font-sans antialiased overflow-hidden">
      ...
      <div className="flex flex-1 flex-col p-4 md:p-6 min-h-0 bg-[#fffdfa]">
        ...
        {/* CONTAINER SCROLL UTAMA KATALOG */}
          ...
          {isLoading ? (
            ...
            <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4 pb-4">
              {filteredProducts.map((product) => (
                <div
                  key={product.id}
                  className="bg-white border border-orange-100/30 rounded-[1.8rem] overflow-hidden shadow-sm flex flex-col justify-between cursor-pointer transition-all hover:border-[#e37b56]/50 hover:scale-[1.01] group"
                  onClick={() => {
                    addItem({
                      id: product.id,
                      name: product.name,
        ...
        <div className="flex items-center justify-between pb-3 border-b border-orange-100/20 flex-shrink-0">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-[10px] text-zinc-400 font-medium">{cart.length} macam produk terpilih</p>
            </div>
          </div>
          {cart.length > 0 && (
            <button onClick={clearCart} className="text-[10px] font-bold text-red-400 hover:text-red-500 transition-colors">
              Clear All
            </button>
          )}
        </div>
            ...
            cart.map((item) => (
              <div key={item.id} className="p-3 bg-[#fffdfc] border border-orange-100/10 rounded-2xl flex items-center justify-between text-xs gap-3">
                ...
                <div className="flex items-center bg-white border border-orange-100/30 rounded-xl px-1 py-0.5 shadow-sm flex-shrink-0">
                  <Button variant="ghost" size="icon" className="h-5 w-5 rounded-lg text-zinc-400 hover:text-[#e37b56]" onClick={() => (item.qty === 1 ? removeItem(item.id) : updateQty(item.id, item.qty - 1))}>
                    <Minus className="h-3 w-3" />
                  </Button>
                  <span className="w-6 text-center font-bold font-mono text-zinc-800 text-xs">{item.qty}</span>
                  <Button variant="ghost" size="icon" className="h-5 w-5 rounded-lg text-zinc-400 hover:text-[#e37b56]" onClick={() => updateQty(item.id, item.qty + 1)}>
        ...
        <DialogContent className="sm:max-w-md bg-white border border-orange-100/30 rounded-[2.5rem] p-6 shadow-2xl">
          ...
          <div className="grid grid-cols-2 gap-3 py-2">
            ...
            <button
              type="button"
              className={`h-20 rounded-2xl border flex flex-col gap-1.5 items-center justify-center transition-all ${
                paymentMethod === "qris" ? "bg-[#e37b56] border-[#e37b56] text-white shadow-sm" : "bg-white border-orange-100/30 text-zinc-600 hover:border-orange-200"
              }`}
